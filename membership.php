...
<?php
    ...
    // get the user name, phone number, email and subscription type from the form if it exists
    if (isset($_POST['name']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['subscription'])) {
        $name = $_POST['name'];
        $phone = $_POST['phone'];
        $email = $_POST['email'];
        // insert the email into the database if it doesn't exist
        $sql = "INSERT INTO subscribers (name, phone, email) VALUES ('$name', '$phone', '$email')";
